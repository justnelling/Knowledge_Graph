...
# * This is to template the output answer from the LLM
...
if user_input:
    ...
    with col2:
        if cypher_query:
            st.text_area("Last Cypher Query", value=cypher_query,
                         key="_cypher", height=240)
    with col3:
        if database_results:
            st.text_area("Last Database Results",
                         value=database_results, key="_database", height=240)
